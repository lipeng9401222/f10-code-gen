...
- `gen-api-doc.mjs`：standalone 模式下从 mock 反推接口文档的端到端能力
...
├── mock/demo/order.mock.ts        # gen-api-doc 输入：标准 defineMock 形态
...
# 期望：6 个接口 + 8 个字段（name / amount / status / customerName / customerPhone / createTime / remark / id）
```

## 注意

- 本目录被 `scripts/smoke-test.mjs` 的 `walk()` 主动跳过，不会触发占位符残留检查
- `docs/api/demo/` 是 gen-api-doc 的产物目录，可以随时重新生成；如果不想纳入 git，可以加到上层 `.gitignore`
