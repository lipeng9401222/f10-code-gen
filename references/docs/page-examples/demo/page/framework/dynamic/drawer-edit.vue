    ...
    <ep-layout-manager class="bg-fill rounded">
      ...
      <template #bottom>
        <div class="flex justify-end py-l px-xxl">
          <e-button @click="handleSave">修改保存</e-button>
          <e-button type="primary" @click="handleSubmit">提交信息</e-button>
        </div>
      </template>
    </ep-layout-manager>
  </div>
...
const openComponentDialog = (title) => {
  proxy?.$drawer(
    ...
    () => {
      return h(
        defineAsyncComponent(() => import('@/views/demo/page/framework/dynamic/drawer-edit-dialog.vue')),
        {
          ouGuid: model.ouGuid,
