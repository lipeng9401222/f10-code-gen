<template>
  ...
  <ep-layout-manager class="fui-page" v-loading="model.global.state.loading">
    ...
    <template #main>
      <div class="max-h-full flex justify-between pt-m pr-xxl pb-xxl pl-xxl flex-1">
        <div class="grid-transfer-panel rounded border bg-white flex flex-col pb-xl">
          <e-toolbar class="px-xl pb-l" :space-gap="0">
            <template #title>
        ...
        <div class="grid-transfer-panel rounded border bg-white flex flex-col pb-xl">
          <e-toolbar class="px-xl pb-l">
            <template #title>
              <e-toolbar-title class="text-lg fw-md">
                <span title="已选择(已选数量/可选上限)">已选择({{ rightTableTotal }}/{{ model.leftGridList.data.total || 0 }})</span>
              </e-toolbar-title>
            </template>
            <template #actions>
              <e-tooltip content="清空" placement="top">
                <e-button class="w-sm h-sm" :icon="Delete" text @click="emptyRight" type="danger" ghost />
              </e-tooltip>
          ...
          <ep-data-grid
            class="flex-1 flex flex-col px-m right-table"
            ref="rightTableRef"
            id-field="id"
            :data="rightTableData"
...
<script setup>
import { ref, watch, computed, onMounted, inject, onActivated } from 'vue';
import { EpDataGrid, Utils, Hooks } from '@epframe/eui-core';
import { Delete, Refresh, Rollback } from '@epoint-fe/eui-icons';
...
const rightSelectedChange = (selectedRowKeys, selectedRows, row) => {
  if (row) {
    model.rightSelectedRowKeys.push(row.id);
  } else {
    // 处理选中项的变更
...
// 清空右侧列表
const emptyRight = () => {
  EMessageBox.confirm('确定要清空吗？', '警告', {
    type: 'warning',
    confirmButtonText: '确定',
    cancelButtonText: '取消'
  })
    .then((data) => {
      const keys = rightTableData.value.map((item) => item.id);
      rightSelectedChange(keys);
    })
    .catch(() => {
      // 用户取消了重置操作
    });
...
const rightColumnList = ref([
  ...
  },
  {
    ...
    action: {
      ...
      defaultShowItems: 2, // 默认展示的操作项数量，默认值为 4
      // 操作项列表
      items: [
        {
          type: 'primary',
          ...
          // 操作项的点击事件处理函数
          onClick: (row) => {
            rightSelectedChange(null, null, row);
          },
          // 操作项是否可见的判断函数
          visible: (row) => true,
          // 操作项是否禁用的判断函数
...
// 监听 rightTableData 改变,添加index
watch(
  () => rightTableData.value,
  () => {
    rightTableData.value.forEach((item, index) => {
      item.index = ++index;
    });
  }
);
...
.grid-transfer-panel {
  width: calc((100% - var(--e-space-l)) / 2);
