  ...
  <ep-layout-manager class="eui-page" v-loading="model.global.state.loading" :top-config="{ contentClass: 'eui-top-section', showDivider: false }">
    <template #top>
      <e-toolbar title-block>
        <template #title>
          <e-toolbar-title>
            <e-dropdown @command="handleCommand" @visible-change="onVisibleChange" v-show="selectCommand">
              <div class="flex justify-start items-center h-sm">
                <span class="h-md text-xl leading-xl mr-m py-xs cursor-pointer">{{ selectCommand }}</span>
                <e-icon size="14">
                  <arrow-down class="transition" :style="{ transform: !commandVisible ? 'rotateZ(0)' : 'rotateZ(-180deg)' }" />
                </e-icon>
              </div>
              <template #dropdown>
                <e-dropdown-menu>
                  <e-dropdown-item v-for="item in model.commandList.data" :key="item.value" :command="item" :class="{ 'text-brand': item.value === model.titleId }">
                    {{ item.label }}
                  </e-dropdown-item>
                </e-dropdown-menu>
              </template>
...
import { Setting, ArrowDown, ArrowUp } from '@epoint-fe/eui-icons';
...
const model = Utils.defineDataModel(() => {
  ...
  // 下拉菜单命令数据模型
  const commandList = useListModel('/showcase/f10-demo/commandList', {
    labelField: 'label', // 显示文本字段
    lazy: false // 懒加载
  });
...
// 标题选择相关
const selectCommand = ref('');
// 处理下拉菜单命令
const handleCommand = (command) => {
  selectCommand.value = command.label; // 更新标题选择模型
  model.titleId = command.value; // 更新列表
};
// 当下拉菜单出现/消失时触发
const onVisibleChange = (val) => {
  commandVisible.value = val; // 更新下拉菜单可见性
};

// 监听组织节点 GUID 变化，刷新表单数据
watch(
  () => model.commandList.data,
  (newVal) => {
    if (newVal && newVal.length > 0) {
      const command = newVal[0];
      selectCommand.value = command.label; // 更新标题选择模型
      model.titleId = command.value; // 更新列表
    }
  },
