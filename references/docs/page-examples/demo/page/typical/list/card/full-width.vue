      ...
      <e-toolbar title-block="false">
        ...
        <template #filter="{ opened }">
          ...
          <div class="inline-flex">
            ...
            <e-tooltip content="切换为通栏视图" placement="top">
        ...
        >
          <template #bodyCell="{ column, text }">
            <template v-if="column.dataIndex === 'status'">
              <e-tag v-if="text >= 1" effect="Soft" size="small" :type="meetingStatusMap[text]?.cls" round class="text-primary">{{ meetingStatusMap[text]?.label || '' }}</e-tag>
          ...
          <template #bodyRow="{ record, selectionNode, actionNode }">
            <e-card class="card-item flex px-xl p-l" @click="onRowClick($event, record)" :title="cardTitle" shadow="hover">
              <div class="card-body w-full flex justify-center items-center">
                <component class="card-selection mr-xl" v-if="selectionNode && showCardCheckbox" :is="selectionNode" />
                <e-avatar class="flex items-center mr-xl" v-if="showCardAvatar" shape="square" :size="50" :src="record.avatarUrl" />
                <div class="row-info flex-1">
                  <div class="row text-lg">
                    <p>{{ record.name }}</p>
                  </div>
                  <div class="other-info text-base text-secondary leading-base mt-s">
                    <span>{{ record.date }}</span>
                    <e-divider direction="vertical" />
                    <span>线下{{ record.room }}</span>
                    <e-divider direction="vertical" />
                    <span>线上ID：{{ record.online_id }}</span>
                  </div>
                </div>
                <div class="row-info flex justify-center items-center gap-xl">
                  <div class="col-status flex justify-center items-center">
                    <e-tag v-if="record.status >= 1" size="small" effect="Soft" :type="meetingStatusMap[record.status]?.cls" round class="text-primary">{{ meetingStatusMap[record.status]?.label || '' }}</e-tag>
                  </div>
                  <div class="col-avatar flex items-center">
...
const cardTitle = ref('点击选中/取消选中当前行');
// 显示卡片复选框
const showCardCheckbox = ref(false);
// 显示卡片头像
const showCardAvatar = ref(false);
// 表格视图 table,card
...
// NOTE: table 组件在卡片层又设置了背景色，无法直接设置透明，只能通过增加一个 wrapper 类来覆盖样式
.custom-eui-table {
  > :deep(.e-table-card) {
