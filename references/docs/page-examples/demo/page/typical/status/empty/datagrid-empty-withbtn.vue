<template>
  <ep-layout-manager class="eui-page" v-loading="model.global.state.loading" :top-config="{ contentClass: 'eui-top-section', showDivider: false }">
    <template #top>
      <e-toolbar title="工单列表" title-block="true">
        <template #button>
    ...
    <template #main>
      <div class="eui-main-section">
        <ep-data-grid
          ...
          config-id="datagrid"
          :key="tableKey"
          :class="!model.gridList.total ? 'empty-table' : ''"
        >
          ...
          <template #emptyText>
            <e-empty>
              <e-button type="primary" @click="addProject">创建项目</e-button>
            </e-empty>
          </template>
        </ep-data-grid>
      </div>
...
const model = Utils.defineDataModel(() => {
  ...
  // 表格数据模型
  const gridList = Hooks.useTableModel('/showcase/f10-demo/gridEmpty', {
    idField: 'id',
