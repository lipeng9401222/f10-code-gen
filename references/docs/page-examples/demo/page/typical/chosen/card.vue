  ...
  <ep-layout-manager class="eui-page" v-loading="model.global.state.loading" :top-config="{ contentClass: 'eui-top-section', showDivider: false }">
    <template #top>
      <e-toolbar title-block>
        <template #title>
          <e-toolbar-title title="选模板/卡片列表"></e-toolbar-title>
        </template>
        <template #button>
...
const openBaseDialog = () => {
  proxy?.$dialog(
    ...
    () => {
      return h(
        defineAsyncComponent(() => import('@/views/demo/page/typical/chosen/card-list.vue')),
        {
          // 弹窗props数据
          data: {}
        }
...
const openBulkDialog = () => {
  proxy?.$dialog(
    ...
    () => {
      return h(
        defineAsyncComponent(() => import('@/views/demo/page/typical/chosen/card_bulk.vue')),
        {
          // 弹窗props数据
          data: {}
        }
