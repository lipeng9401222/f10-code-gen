<template>
  ...
  <ep-layout-manager class="eui-page" v-loading="model.global.state.loading" :top-config="{ contentClass: 'eui-top-section px-xxl pt-m pb-0', showDivider: false }">
    <template #top>
      <e-toolbar>
        <template #title>
          <e-toolbar-title>
            <span class="text-lg fw-md">已选择({{ model.selectedRowKeys.length }})</span>
          </e-toolbar-title>
        </template>
    ...
    <template #main>
      <div class="eui-main-section p-xxl pt-0">
        <ep-data-grid
          ref="tableRef"
          class="eui-table"
