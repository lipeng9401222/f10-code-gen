<template>
  <ep-layout-manager class="eui-page" v-loading="model.global.state.loading" :top-config="{ contentClass: 'eui-top-section', showDivider: false }">
    <template #top>
      <e-toolbar title="抽屉弹窗" title-block="true">
        <template #button>
 ...
 * @param {boolean} withModal - 是否显示遮罩
 */
const openDrawer = (title, withModal = true) => {
  proxy?.$drawer(
    {
      title,
      size: '500px',
    ...
    () => {
      return h(
        defineAsyncComponent(() => import('@/views/demo/page/typical/dialog/form.vue')),
        {
          rowGuid: '',
          showCancel: true,
          showNav: false,
...
// 有遮罩抽屉
const openDrawerWithModal = () => openDrawer('新增采购信息（有遮罩）', true);
// 无遮罩抽屉
const openDrawerWithoutModal = () => openDrawer('新增采购信息（无遮罩）', false);
// 回到第一步
const backFirst = () => {};
// 终止流程
...
const toolbarBtnList = ref([
  {
    ...
    items: [
      {
        content: '抽屉弹窗（模态）',
        onClick: () => openDrawerWithModal()
      },
      {
        content: '抽屉弹窗（非模态）',
        onClick: () => openDrawerWithoutModal()
      }
    ]
  },
  {
