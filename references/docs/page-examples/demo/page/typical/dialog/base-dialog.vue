<template>
  <ep-layout-manager class="eui-page" v-loading="model.global.state.loading" :top-config="{ contentClass: 'eui-top-section', showDivider: false }">
    <template #top>
      <e-toolbar title="基础弹窗" title-block="true">
        <template #button>
 ...
 */
const openDialog = (title, width, height) => {
  proxy?.$dialog(
    {
      title,
      width,
...
// 小号弹窗 (400px)
const openSmallDialog = () => openDialog('新增采购信息（小）', 400, 620);
// 中号弹窗 (800px)
const openMediumDialog = () => openDialog('新增采购信息（中）', 800, 600);
// 大号弹窗 (1200px)
const openLargeDialog = () => openDialog('新增采购信息（大）', 1200, 600);
// 回到第一步
const backFirst = () => {};
// 终止流程
...
const toolbarBtnList = ref([
  {
    ...
    items: [
      {
        content: '小号弹窗(400px)',
        onClick: () => openSmallDialog()
      },
      {
        content: '中号弹窗(800px)',
        onClick: () => openMediumDialog()
      },
      {
        content: '大号弹窗(1200px)',
