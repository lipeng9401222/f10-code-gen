<template>
  <div ref="layoutRef" class="h-full">
    <ep-layout-manager class="eui-page transition" :class="{ 'opacity-0': !pageVisible }" v-loading="model.global.state.loading" :top-config="{ contentClass: 'eui-top-section', showDivider: false }">
      <template #top>
        <e-toolbar title="嵌入式弹窗" title-block="true">
          <template #button>
  ...
  proxy?.$modal(
    {
      withHeader: false,
      mode: 'embed',
      target: layoutRef.value, // 以当前页面为基准定位
      onOpen: () => (pageVisible.value = false), // 打开弹窗时隐藏页面
      beforeClose: (done) => {
        pageVisible.value = true; // 关闭弹窗时显示页面
        done();
      },
    ...
    () => {
      return h(
        defineAsyncComponent(() => import('@/views/demo/page/typical/dialog/embed-dialog-page.vue')),
        {
          rowGuid: '',
          showBottom: true
        }
...
// 打开全屏弹窗
const openEmbedModal = () => openModal();
// 回到第一步
const backFirst = () => {};
// 终止流程
...
const toolbarBtnList = ref([
  {
    type: 'primary',
    onClick: async () => openEmbedModal(),
    content: '新增采购信息（嵌入式弹窗）'
  },
  {
    type: 'default',
