<template>
  <ep-layout-manager class="fui-page" v-loading="model.global.state.loading">
    <template #top>
      <div>
        <e-toolbar title="漫游式引导" title-block="true">
          <template #actions>
            <e-button-group>
        ...
        >
          <template #bodyCell="{ column, text, record }">
            <template v-if="column.dataIndex === 'reviewState'">
              <e-tag :id="'tag-' + record.id" :type="reviewStateData[text - 1].cls" round class="text-primary tag-class">{{ reviewStateData[text - 1].text }}</e-tag>
            </template>
          </template>
        </ep-data-grid>
      </div>
  ...
  <!-- 如果id存在的话，则使用id作为target -->
  <!-- <e-tour-step target="#addPurchase" :icon="Menu" title="自定义搜索范围" description="一次输入就能检索多项字段！" /> -->
  <e-tour v-model="tourOpen" :show-arrow="false" :target-area-clickable="false">
    <e-tour-step :target="addPurchaseEl" :icon="Menu" title="自定义搜索范围" description="一次输入就能检索多项字段！" />
    <e-tour-step :target="searchAllEl" :icon="Setting" title="自定义搜索范围" description="一次输入就能检索多项字段！" />
    <e-tour-step :target="tableRef?.$el" :icon="MoreFilled" title="自定义搜索范围" description="一次输入就能检索多项字段！" />
    <e-tour-step :target="tagFirstEl" :icon="Save" title="自定义搜索范围" description="一次输入就能检索多项字段！" />
  </e-tour>
</template>
<script setup>
...
import { Setting, Menu, MoreFilled, Save } from '@epoint-fe/eui-icons';
...
// 定义dom元素引用
// 按钮组某个按钮
const addPurchaseEl = ref(null);
// 选择表格
const datagridEl = ref(null);
// 选择筛选条件内的搜索按钮
const searchAllEl = ref(null);
// 选择表格内某个通过id或class指定的元素
const tagFirstEl = ref(null);
...
onMounted(() => {
  ...
  nextTick(() => {
    timer.value = setTimeout(() => {
      addPurchaseEl.value = toolbarBtnsRef.value.$el.querySelector('#addPurchase');
      datagridEl.value = tableRef.value.$el.querySelector('#datagrid');
      searchAllEl.value = toolbarSearchRef.value.$el.querySelector('.search-wrapper');
      tagFirstEl.value = tableRef.value.$el.querySelector('.tag-class:first-child');
      tourOpen.value = true;
    }, 600);
  });
});
